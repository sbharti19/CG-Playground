...
const Input: React.FC = () => {
    ...
    const [user] = useAuthState(auth); // Get user from Firebase Auth
    ...
    const addEmoji = (e: any) => {
        ...
        let emoji = String.fromCodePoint(...codesArray.map(code => Number(code)));
        setInput(input + emoji);
    };

    const sendPost = async () => {
        if (loading || !user) {
            if (!user) toast.error("You must be logged in to post.", { position: "top-left", theme: "dark" });
            return;
        }
        ...
        try {
            const docRef = await addDoc(collection(db, 'posts'), {
                id: user.uid,
                username: user.displayName || user.email,  // Fallback to email if displayName is unavailable
            ...
            toast.error("Failed to submit post. Please try again.", { position: "top-left", theme: "dark" });
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className={`mt-4 px-4 ${loading && "opacity-60"}`}>
            ...
            <div className='grid grid-cols-[48px,1fr] gap-4'>
                ...
                <div>
                    <img className='h-12 w-12 rounded-full object-contain' src="/avatar.png" alt="" />
                </div>
